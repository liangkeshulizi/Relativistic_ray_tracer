...
if __name__ == '__main__':
    ...
    light_pos = vec3(0, 0, 0)         # 点光源位置
    center = vec4(0, 0, 0, 0)           # 摄像机位置
    focal_length= 200

    shape4 = RectangularPrism(1, 1, 1, .04, diffuse_color_function= lambda p: BILIBILIBLUE_A)
    beta = (0, 0, -0.6)
    movingobjects= [MovingObject(shape4, beta, vec4(0, 0, 0, 2))]#[MovingObject(shape2, beta, offset2, Material(500)), MovingObject(shape3, (0,0,0), offset3, None)]
    ...
    t_start= time.time()
    frames= 250
    for shot_time, frame_count in np.linspace([1,1],[4.5,frames], frames):
