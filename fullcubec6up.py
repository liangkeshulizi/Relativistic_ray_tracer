...
shape_ground = Plane(vec3(0, -0.5, 0), vec3(0, 1, 0), diffuse_color_function=lambda p:GREY)
shape_cube= Cube(1, 1, 1, diffuse_color_function= lambda p: BILIBILIBLUE)
shape_ball= Sphere(.5)
...
movingobjects= [
                ...
                MovingObject(shape_cube, (0.6,0,0), vec4(0,0,0,0), Material(False)),
                MovingObject(shape_ground, (0,0,0), vec4(0,0,0,0), None)
                ]
...
z= np.repeat(np.linspace(S[1], S[3], height), width) #这里调整摄像机方向
y= -focal_length
origin_to_image_times= vec3(x,y,z).norm()

t_start= time.time()
frames= 250
for shot_time, frame_count in np.linspace([-2.5,1],[10,frames], frames):
    ...
    filename= ".\\render4\\%i.png" % frame_count
